...
module TXG.Fake where

import Control.Monad
import System.Random
import Data.Bifunctor (second)
import Data.Text hiding (map,length,split)
...
oneof xs = fromRange (0, length xs - 1) >>= (xs !!)
...
elements :: [a] -> FGen a
elements [] = error "Fake.elements used with empty list"
elements xs = (xs !!) <$> fromRange (0, length xs - 1)
...
phraseText = Data.Text.unwords . map unSingleWord . unPhrase
